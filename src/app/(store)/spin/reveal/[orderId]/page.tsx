...
import { finalizeRandomCardCheckout } from '@/lib/supabase/random-card-actions'

export const dynamic = 'force-dynamic'

export default async function SpinRevealPage({
  params,
}: {
  params: Promise<{ orderId: string }>
}) {
  const { orderId } = await params
  const result = await finalizeRandomCardCheckout(orderId)
    ...
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-2xl flex-col items-center justify-center px-4 text-center">
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row">
          <Link href="/spin">
            <Button>Back to Mystery Cards</Button>
          </Link>
